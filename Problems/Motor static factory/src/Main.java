...
class MotorStaticFactory {
    ...
    public static Motor make(char type, String model, long power) {
        ...
        switch (type) {
            case 'P':
                return motor = new PneumaticMotor(model, power);
            case 'H':
                return motor = new HydraulicMotor(model, power);
            case 'E':
                return motor = new ElectricMotor(model, power);
            case 'W':
                return motor = new WarpDrive(model, power);
            default:
                return null;
        }
    }
